...
//  let pp = mod_vec3(p ,vec3<f32>(2.0)) ; 
  let k = .5; // スムージング係数
  var d = 1e20 ;
  let d1 = sphereSDF(p, vec3<f32>(sin(time)*0.8, 0.3,cos(time)*0.8), 0.2);
  d = smin(d,d1,k) ;
  let d2 = sphereSDF(p, vec3<f32>(sin(time*2)*0.6, 0.2,cos(time*2)*0.6), 0.25);
  d = smin(d,d2,k) ;
  let d3 = sphereSDF(p, vec3<f32>(sin(-time*3)*0.5, 0.3,cos(-time*3)*0.5), 0.3);
  d = smin(d,d3,k) ;
  let d4 = sphereSDF(p, vec3<f32>(sin(-time*4)*0.5, 0.2,cos(-time*4)*0.5), 0.2);
  d = smin(d,d4,k) ;
  ...
  if (true && tryBoundingSphere(ray, hit, vec3<f32>(0.0, 0.0, 0.0), 3.)) {
    ...
    if(tryDistanceFieldObject(ray,hit,ts)) {
      (*hit).material = Mat_mirror(vec3<f32>(0.85, 0.8, 0.1)*2.);
        ...
        if (modFloat(((*hit).position.x + (*hit).position.z - 0.) / 0.2, 1.0) < 0.5) {
          (*hit).material = Mat_brdf(hsl2rgb(vec3<f32>(0., 0.9, .5)),0.6,0.2,1.);
        } else {
          (*hit).material = Mat_brdf(hsl2rgb(vec3<f32>(0.6, 0.9, 0.2)),0.3,0.7,1.);
        }
