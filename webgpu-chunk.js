...
const SHADER_CHUNK_FILES = [
  'path1-environment.wgsl',
  'path1-scene.wgsl',
  'path1-common.wgsl'
];

const STEREO_IPD = 0.; // if not zero, stereo rendering IPD
  ...
  const controller = {
    async renderFrame(timeSec) {
      ...
      if (STEREO_IPD === 0) {
        ...
        uniformValues[1] = -STEREO_IPD * 0.5;
        commitUniforms(16);
        pass.setViewport(0, 0, halfWidth, currentHeight, 0, 1);
        pass.draw(3, 1, 0, 0);

        uniformValues[1] = STEREO_IPD * 0.5;
        commitUniforms(16);
        pass.setViewport(halfWidth, 0, halfWidth, currentHeight, 0, 1);
        pass.draw(3, 1, 0, 0);
      }
