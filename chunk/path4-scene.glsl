...
void intersectScene(Ray ray, inout HitInfo hit) {
  ...
  if(tryBoundingSphere(ray,hit,vec3(0.,0.,0.),3.)) {
    ...
    if( trySphere(ray, hit, centerC, 0.6)) hit.material = Mat_brdf(vec3(1.55, 1.5, 0.0),1.0,0.,1.);
  ...
  //光源判定
  vec3 lightEmission = vec3(4.0, 2.0, 9.0) * 5.65 ;
  if( trySphere(ray, hit, vec3(2.0, 6.5, 0.0), 1.)) hit.material = Mat_light(lightEmission) ;
}
